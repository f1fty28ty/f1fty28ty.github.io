...
// Hub filter tables: chip toggles + click-anywhere-in-row navigation,
// used by the realm hubs (freestyle/freeride/powder/carving) and drills.
function initHubFilterTables() {
  document.querySelectorAll('[data-hub-table]').forEach((table) => {
    const wrap = table.closest('.hub-table-wrap') || table.parentElement
    const filterId = table.getAttribute('data-hub-table')
    const filterBar = document.querySelector(`[data-hub-filter="${filterId}"]`)
    const rows = Array.from(table.querySelectorAll('tbody tr[data-category]'))
    const emptyRow = table.querySelector('.hub-table__empty-row')

    // Row navigation: click anywhere in the row (except the link itself,
    // which already navigates on its own) follows that row's trick link.
    rows.forEach((row) => {
    ...
    const chips = Array.from(filterBar.querySelectorAll('.hub-filter__chip'))
    const resetBtn = filterBar.querySelector('.hub-filter__reset')

    function applyFilter() {
      // keep aria-pressed in step with the visual state so screen readers hear it too
      chips.forEach((c) => c.setAttribute('aria-pressed', String(c.classList.contains('is-active'))))
      const active = new Set(
        chips.filter((c) => c.classList.contains('is-active')).map((c) => c.dataset.category)
      )
      let visibleCount = 0
      rows.forEach((row) => {
        const show = active.has(row.dataset.category)
